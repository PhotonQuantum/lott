...
theorem map_singleton_flatten (xs : List α) : (xs.map fun x => [f x]).flatten = xs.map f :=
  match xs with
  | [] => rfl
  | x :: xs' => by simp [xs'.map_singleton_flatten]
...
theorem get!_mem [Inhabited α] {as : List α} (h : i < as.length) : as[i]! ∈ as := by
  rw [getElem!_pos]
  exact getElem_mem h
      ...
      · case neg h =>
        ...
        · case neg h' =>
          ...
          exact two_le_count_of_get!_eq_of_ne (Nat.add_lt_add_iff_right.mp iltlen)
            (Nat.add_lt_add_iff_right.mp jltlen) eq (fun h => ne (congrArg Nat.succ h))
    · case isFalse h =>
      by_cases i = 0
      · case pos h' =>
        cases h'
        rw [List.getElem!_cons_zero, BEq.refl] at h
        nomatch h
      · case neg h' =>
        by_cases j = 0
        · case pos h'' =>
          cases h''
          rw [eq, List.getElem!_cons_zero, BEq.refl] at h
          nomatch h
        · case neg h'' =>
          let ⟨k, eq'⟩ := Nat.exists_eq_succ_of_ne_zero h'
          cases eq'
          let ⟨k', eq''⟩ := Nat.exists_eq_succ_of_ne_zero h''
          cases eq''
          rw [List.getElem!_cons_succ, Nat.add_zero]
          exact two_le_count_of_get!_eq_of_ne (Nat.add_lt_add_iff_right.mp iltlen)
            (Nat.add_lt_add_iff_right.mp jltlen) eq (fun h => ne (congrArg Nat.succ h))
...
theorem get!_zip [Inhabited α] [Inhabited β] {l₁ : List α} {l₂ : List β}
  ...
  | a :: l₁', b :: l₂' =>
    rw [zip_cons_cons]
    match i with
    | 0 => rw [List.getElem!_cons_zero, List.getElem!_cons_zero, List.getElem!_cons_zero]
    | i' + 1 =>
      rw [List.getElem!_cons_succ, List.getElem!_cons_succ, List.getElem!_cons_succ]
